...
pitch = 360/3;
  ...
  difference(){
    union(){
      zmove(base_height/2 + clamp_clearence/2) zcyl(h=base_height - clamp_clearence, d=base_dia, $fn=100, rounding2=0.9*u) {
  ...
  for (i = [0:3-1])
    zrot(i*pitch+60)
      move([13.26,-5,2.99]) prismoid(size1=[21.3,10], size2=[10,5], h=5, shift=[5,-2.5]);
  }
}
